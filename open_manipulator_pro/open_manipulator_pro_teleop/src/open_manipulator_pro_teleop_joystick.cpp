...
{
  goal_joint_space_path_from_present_client_ = node_handle_.serviceClient<open_manipulator_msgs::SetJointPosition>("goal_joint_space_path_from_present"); //추가, 현재 위치에서 joint의 값을 델타 위치만큼 manipulator의 이동을 요청하는데 사용
...
{
  joint_states_sub_ = node_handle_.subscribe("open_manipulator_pro/joint_states", 10, &OpenManipulatorTeleop::jointStatesCallback, this);
  kinematics_pose_sub_ = node_handle_.subscribe("open_manipulator_pro/kinematics_pose", 10, &OpenManipulatorTeleop::kinematicsPoseCallback, this);
...
{
  if (msg->axes.at(0) >= 0.9) setGoal("y+");
  else if (msg->axes.at(0) <= -0.9) setGoal("y-");
  else if (msg->axes.at(1) >=  0.9) setGoal("x+");
  else if (msg->axes.at(1) <= -0.9) setGoal("x-");
  else if (msg->axes.at(5) == 1) setGoal("z+");
  else if (msg->axes.at(5) == -1) setGoal("z-");
  else if (msg->buttons.at(4) == 1) setGoal("home");
  ...
  if (with_gripper_)
  {
    if (msg->buttons.at(10) == 1) setGoal("gripper close");
    else if (msg->buttons.at(11) == 1) setGoal("gripper open");
  }
}
}
...
bool OpenManipulatorTeleop::setJointSpacePathFromPresent(std::vector<std::string> joint_name, std::vector<double> joint_angle, double path_time)// 추가
{
...
void OpenManipulatorTeleop::setGoal(const char* str)
{
  ...
  else if(str == "joint6+")// 추가, end effector 오른쪽 방향으로 회전
  {
  ...
  else if(str == "joint6-")// 추가, end effector 왼쪽쪽 방향으로 회전
  {
